  ...
  if (form) {
    ...
    form.addEventListener('submit', (e) => {
      ...
      fetch(form.action, {
        ...
        .then(async (res) => {
          // Read whatever came back so we can log it
          let payload = null;
          try { payload = await res.json(); } catch (_) { payload = await res.text().catch(() => null); }
          console.log('[enquiry] Formspree response:', { status: res.status, ok: res.ok, payload });
          ...
          setStatus(`Thanks ${firstName} — your enquiry's in. I'll reply within one working day.`, 'success');
        })
        .catch((err) => {
          console.error('[enquiry] Send failed:', err, err && err.payload);
          if (submitBtn) submitBtn.disabled = false;
          if (labelEl)   labelEl.textContent = originalLabel || 'Send enquiry';
          setStatus('Something went wrong sending that. Please email hello@benmann.co.nz directly.', 'error');
        });
    });
  }
